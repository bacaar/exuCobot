...
-> reads csv with coefficients generated by c++ and evaluates them  -> data1
-> reads csv with parameters for trajectory generation (used by c++), recreates them with pyhton functions -> data2
...
def main():
    ...
    t1, data1 = evaluateCoefficients()
    t2, data2 = offlineCalculation(polyOrder)
    ...
    fig.legend(["trajectory c++", "trajectory pyhton"])
